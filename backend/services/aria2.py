...
class Aria2Service:
    ...
    async def get_all(self) -> List[Aria2DownloadStatus]:
        methods = [
            {"methodName": "aria2.tellActive", "params": [self._keys()]},
            {"methodName": "aria2.tellWaiting", "params": [0, 1000, self._keys()]},
            {"methodName": "aria2.tellStopped", "params": [0, 1000, self._keys()]},
        ]
        results = await self._call("system.multicall", [methods])
